...
\author{
    Jaisel Singh, Huan Gu, Qinghua He \\
    \\
    Columbia University \\
...
\begin{enumerate}
    \item \textbf{Initialization:} Set initial state $\bar{\state}_0$ and initial covariance $\bar{\covariance}_0$
    \item \textbf{For each time step $t = 1, 2, \ldots$:}
    \begin{enumerate}
        \item \textbf{Prediction Step:}
        \begin{itemize}
            \item Predict state: $\bar{\state}_t = g(\state_{t-1}, \control_t)$
            \item Predict covariance: $\bar{\covariance}_t = \bm{G}_t \covariance_{t-1} \bm{G}_t^\top + \bm{R}_t$
        \end{itemize}
        \item \textbf{Update Step (for each observation $\measurement_t^i$):}
        \begin{itemize}
            \item Compute Kalman gain: $\kalman_t = \bar{\covariance}_t \bm{H}_t^\top (\bm{H}_t \bar{\covariance}_t \bm{H}_t^\top + \noise_t)^{-1}$
            \item Update state: $\state_t = \bar{\state}_t + \kalman_t (\measurement_t^i - h(\bar{\state}_t))$
            \item Update covariance: $\covariance_t = (\bm{I} - \kalman_t \bm{H}_t) \bar{\covariance}_t$
        \end{itemize}
    \end{enumerate}
\end{enumerate}
...
def predict(self, dx_local, dy_local, dtheta):
    ...
    Motion model: robot moves by (dx, dy) in its local frame, then rotates by dtheta.
...
def detect_corners(self, line_segments):
    ...
    for i in range(len(line_segments)):
        for j in range(i + 1, len(line_segments)):
            ...
            if 80 <= angle <= 100:  # 90 degree corner range
...
def associate_landmark(self, z_range, z_bearing):
    ...
    for i in range(self.num_landmarks):
        ...
        # Mahalanobis distance: mahal_dist = innovation^T S^{-1} innovation
        try:
...
def add_landmark(self, z_range, z_bearing):
    ...
    # lx = rx + r*cos(theta + beta), ly = ry + r*sin(theta + beta)
    cos_a = np.cos(global_angle)
    sin_a = np.sin(global_angle)
    
    # Gp: Jacobian w.r.t. robot pose [x, y, theta]
    Gp = np.array([
        [1, 0, -z_range * sin_a],
        [0, 1, z_range * cos_a]
    ])
...
Association Threshold & 1.0 ($\chi^2 \approx 1.0$) & 6.0 ($\chi^2 \approx 6.0$) \\
Max Landmarks & 20 & 30 \\
State Update Limits & $\pm$0.01 m, $\pm$0.01 rad & $\pm$0.05 m, $\pm$0.05 rad \\
