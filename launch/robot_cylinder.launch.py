...
def generate_launch_description():
    ...
    # Set the path to the cylinder world file
    default_world = os.path.join(
        pkg_share, 'worlds', 'cylinder_world.world'
    )

    # Set up model path to find turtlebot3 models
    bookstore_models_dir = os.path.join(pkg_share, 'worlds', 'bookstore', 'models')
    existing_model_path = os.environ.get('GAZEBO_MODEL_PATH', '')
    combined_model_path = (
        f"{bookstore_models_dir}:{existing_model_path}"
        if existing_model_path else bookstore_models_dir
    )
